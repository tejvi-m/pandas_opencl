...
void gpu_add(int* array, int* array1,  int n){
 ...
 // Create the compute kernel in the program we wish to run
  size_t bytes = n*sizeof(int);
  kernel = clCreateKernel(gpu.program, "vecSub", &gpu.err);
  gpu.globalSize = ceil(n/(float)gpu.localSize)*gpu.localSize;

  // Create the input and output arrays in device memory for our calculation
  d_a = clCreateBuffer(gpu.context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR, bytes, array, NULL);
  d_b = clCreateBuffer(gpu.context, CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR, bytes, array1, NULL);
  d_c = clCreateBuffer(gpu.context, CL_MEM_ALLOC_HOST_PTR , bytes, NULL, NULL);
...
void SeriesInt::add(Series* src){
  if(1){
    gpuArithmetic("vecDiv",&(this -> getVec(int())[0]), &(src -> getVec(int())[0]), &(this -> getVec(int())[0]),
